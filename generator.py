...
def generateClass(directory, classNumber, methodsPerClass, mainPackage):
    ...
    filePath = os.path.join(directory, className + ".kt")
    with open(filePath,"w+") as f:
        f.write("package " + mainPackage + "." + directory + "\n")
        f.write("class " + className + " {\n")
        for i in xrange(0, methodsPerClass):
            f.write("fun foo" + str(i) + "(){\n")
            if i > 0:
                f.write("foo" + str(i-1) + "()\n")
            elif classNumber > 0:
                f.write("Foo" + str(classNumber-1) + "().foo" + str(methodsPerClass-1) + "()\n")
            else:
...
if len(sys.argv) < 5:
    ...
    print "mainPackage : %s", mainPackage
